    ...
    try {
      ...
      try {
        ...
        if (familyInfo.success && familyInfo.has_family && familyInfo.members) {
          ...
          // Always include current user + approved family members (use Set to avoid duplicates)
          familyUserIds = [...new Set([currentUser.id, ...approvedMembers.map(m => m.user_id)])];
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="browse" className="space-y-6">
          <div className="relative w-full">
            ...
            <Input placeholder="Search my recipes..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 h-12" maxLength={100} />
          </div>
          ...
          {createMethod === "ai" ? (
            <Card>
              <CardContent className="p-6">
                <div className="flex flex-col gap-3">
                  <Input placeholder="e.g., Chocolate Chip Cookies..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} disabled={generating} className="text-lg h-12" maxLength={100} />
            ...
            <Card>
              <CardContent className="p-6 space-y-6">
                <div><Label>Dish Name *</Label><Input placeholder="e.g., Grandma's Apple Pie" value={manualRecipe.full_title} onChange={(e) => setManualRecipe({ ...manualRecipe, full_title: e.target.value })} className="h-12" maxLength={100} /></div>
                ...
                <div className="space-y-3"><div className="flex justify-between"><Label>Ingredients *</Label><Button onClick={() => setManualRecipe({ ...manualRecipe, ingredients: [...manualRecipe.ingredients, ''] })} size="sm" variant="outline"><Plus className="w-4 h-4 mr-1" />Add</Button></div><div className="space-y-2">{manualRecipe.ingredients.map((ing, idx) => <div key={idx} className="flex gap-2"><Input placeholder="e.g., 2 cups flour" value={ing} onChange={(e) => { const n = [...manualRecipe.ingredients]; n[idx] = e.target.value; setManualRecipe({ ...manualRecipe, ingredients: n }); }} className="flex-1" maxLength={200} />{manualRecipe.ingredients.length > 1 && <Button onClick={() => setManualRecipe({ ...manualRecipe, ingredients: manualRecipe.ingredients.filter((_, i) => i !== idx) })} size="icon" variant="ghost" className="text-red-500"><Trash2 className="w-4 h-4" /></Button>}</div>)}</div></div>
                <div className="space-y-3"><div className="flex justify-between"><Label>Steps *</Label><Button onClick={() => setManualRecipe({ ...manualRecipe, steps: [...manualRecipe.steps, { title: '', instruction: '' }] })} size="sm" variant="outline"><Plus className="w-4 h-4 mr-1" />Add</Button></div><div className="space-y-3">{manualRecipe.steps.map((step, idx) => <Card key={idx} className="border-2 border-orange-200"><CardContent className="p-3 space-y-2"><div className="flex gap-2"><Badge className="bg-orange-500">Step {idx + 1}</Badge><Input placeholder="Title..." value={step.title} onChange={(e) => { const n = [...manualRecipe.steps]; n[idx].title = e.target.value; setManualRecipe({ ...manualRecipe, steps: n }); }} className="flex-1" maxLength={100} />{manualRecipe.steps.length > 1 && <Button onClick={() => setManualRecipe({ ...manualRecipe, steps: manualRecipe.steps.filter((_, i) => i !== idx) })} size="icon" variant="ghost" className="text-red-500"><Trash2 className="w-4 h-4" /></Button>}</div><Textarea placeholder="Instructions..." value={step.instruction} onChange={(e) => { const n = [...manualRecipe.steps]; n[idx].instruction = e.target.value; setManualRecipe({ ...manualRecipe, steps: n }); }} rows={2} maxLength={2000} /></CardContent></Card>)}</div></div>
